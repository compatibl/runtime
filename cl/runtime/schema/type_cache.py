...
class TypeCache:
    ...
    def get_type_name(
        ...
        type_: type,
        *,
        raise_on_unknown_type: bool = True,
    ) -> str:
        """
        Get type name generated by typename(...) method from the type,
        error if the type is not in TypeCache and raise_on_unknown_type is True.

        Args:
            type_: Type for which qual name is generated
            raise_on_unknown_type: Raise an error if the type is not in TypeCache
        """
        result = typename(type_)
        if raise_on_unknown_type and not cls.is_known_type(result):
    ...
    def get_qual_name(
        ...
        *,
        ...
        Args:
            ...
            raise_on_unknown_type: Raise an error if the type is not in TypeCache
        """
        if raise_on_unknown_type and not cls.is_known_type(type_name := typename(type_)):
            raise RuntimeError(f"Type {type_name} not found in TypeCache when calling get_type_name method.")
    ...
    def _add_class(cls, class_: type, *, subtype: str | None = None) -> None:
        ...
        qual_name = cls.get_qual_name(class_, raise_on_unknown_type=False)
