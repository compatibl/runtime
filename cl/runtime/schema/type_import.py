...
class TypeImport:
    ...
    @classmethod
    def qual_name_from_class(cls, class_: Type) -> str:
        """Get fully qualified name in module.ClassName format from the class."""
        return f"{class_.__module__}.{class_.__name__}"

    @classmethod
    def class_from_qual_name(cls, qual_name: str, *, rebuild_cache: bool = True) -> Type:
            ...
            if not module_imported:
                try:
                    ...
                    if rebuild_cache:
                        ...
                        raise RuntimeError(f"Module not found using module name: {module_name}")
    ...
    @classmethod
    def class_from_type_name(cls, type_name: str, *, rebuild_cache: bool = True) -> Type:
        """
        ...
        if (qual_name := cls._qual_name_by_type_name_dict.get(type_name, None)) is not None:
            return cls.class_from_qual_name(qual_name)
    ...
    @classmethod
    def subclasses_of(cls, class_: Type, *, predicate: Callable[[Type], bool]) -> Tuple[Type, ...]:
        ...
        result = {TypeImport.qual_name_from_class(class_): class_} if predicate(class_) else {}
        for subclass in class_.__subclasses__():
            # Recursively update with non-abstract subclasses
            unfiltered = cls.subclasses_of(subclass, predicate=predicate)
            filtered = [x for x in unfiltered if predicate(x)]
            result.update({TypeImport.qual_name_from_class(x): x for x in filtered})
        return tuple(result.values())

    @classmethod
    def superclasses_of(cls, record_type: Type, *, predicate: Callable[[Type], bool]) -> Tuple[Type, ...]:
    ...
    @classmethod
    def records_sharing_key_with(cls, class_: Type, *, predicate: Callable[[Type], bool]) -> Tuple[Type, ...]:
        ...
        if not superclasses:
            ...
            result = cls.subclasses_of(key_type, predicate=predicate)
            return result

    @classmethod
    def cached_classes(cls, *, predicate: Callable[[Type], bool]) -> Tuple[Type, ...]:
        ...
        cached_classes = [cls.class_from_qual_name(qual_name) for type_name, qual_name in sorted_name_pairs]
        ...
        if (existing_class := cls._type_by_type_name_dict.setdefault(type_name, class_)) == class_:
            ...
            if (existing_class := cls._type_by_qual_name_dict.setdefault(qual_name, class_)) != class_:
                ...
                raise RuntimeError(
                    ...
                    f"  - {cls.qual_name_from_class(existing_class)}\n"
                    f"  - {qual_name}\n"
                )
