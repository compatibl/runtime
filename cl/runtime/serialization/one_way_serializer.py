...
class OneWaySerializer:
    ...
    serialize_primitive: bool | None = None
    """Convert primitive types to strings during serialization if set."""

    def to_dict(self, data: Any) -> Any:
        """Serialize a slots-based object to a dictionary without using the schema."""
        ...
        if getattr(data, "__slots__", None) is not None:
            ...
            result = {
                (k if not self.pascalize_keys else CaseUtil.snake_to_pascal_case(k)): (
                    (FormatUtil.format(v) if self.serialize_primitive else v)
            ...
            if is_primitive_collection:
                ...
                result = [
                    "None" if self.serialize_primitive else None if v is None
                    else (FormatUtil.format(v) if self.serialize_primitive else v)
                    if v.__class__.__name__ in _PRIMITIVE_TYPE_NAMES
                    else v.name if isinstance(v, Enum)
                    else self.to_dict(v)
                    for v in data
